...
@section('page-title', 'Statistik Member')
@section('page-subtitle', 'Jumlah member baru mendaftar')
@section('content')
<div class="flex flex-col gap-5">
    ...
    <div class="relative overflow-hidden rounded-xl bg-white border border-neutral-200">
        ...
        <div class="p-4 flex flex-wrap items-end justify-between gap-3">
            @include('admin.statistik._filter-statistik', ['routeName' => 'admin.statistik.member'])

            <a href="{{ route('admin.statistik.member.export', request()->only(['tahun', 'bulan'])) }}"
                title="Export Excel"
    ...
    <div class="relative overflow-hidden rounded-xl bg-white border border-neutral-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3">
            <div class="px-5 py-3.5 flex flex-col gap-0.5">
                ...
                <span class="text-2xl font-semibold tabular-nums text-neutral-800">{{ number_format($totalPeriode, 0, ',', '.') }}</span>
            </div>
            <div class="px-5 py-3.5 flex flex-col gap-0.5 sm:border-l border-neutral-100 border-t sm:border-t-0">
                ...
                <span class="text-2xl font-semibold tabular-nums text-neutral-800">
                    {{ $mode === 'harian' ? \Carbon\Carbon::create($tahun, $bulan, 1)->translatedFormat('F Y') : $tahun }}
                </span>
            </div>
            <div class="px-5 py-3.5 flex flex-col gap-0.5 sm:border-l border-neutral-100 border-t sm:border-t-0">
                <span class="text-xs text-neutral-400 font-medium">Rata-rata per {{ $mode === 'harian' ? 'hari' : 'bulan' }}</span>
                <span class="text-2xl font-semibold tabular-nums text-primary-700">
                    {{ count($values) > 0 ? number_format($totalPeriode / count($values), 1, ',', '.') : 0 }}
                </span>
            </div>
        </div>
    </div>
