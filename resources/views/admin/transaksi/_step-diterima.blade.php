<div class="step-content-{{ $prefix }} hidden" data-step="3">
    ...
    <div class="mb-5">
        <label class="block text-sm font-medium text-neutral-600 mb-2">Cari Buku</label>
        <div class="flex gap-2">
            ...
            <button onclick="cariBukuDiterima('{{ $prefix }}')"
                class="px-4 py-2.5 text-sm rounded-lg bg-primary-50 text-primary-700 hover:bg-primary-100 transition-colors font-medium">
                Cari
            </button>
        </div>
        <p class="text-xs text-neutral-400 mt-1.5">Masukkan ISBN untuk pencarian tepat, atau ketik judul/pengarang</p>
    ...
    <div id="{{ $prefix }}_bukuDiterimaResult" class="hidden">
        <div class="p-4 rounded-xl border border-success-200 bg-success-50 flex items-start gap-3">
            ...
            <button onclick="resetBukuDiterima('{{ $prefix }}')" class="text-xs text-success-600 hover:text-success-800 underline shrink-0">Ganti</button>
        </div>
        <input type="hidden" id="{{ $prefix }}_bukuDiterimaId"/>
    </div>

    <div class="mt-4">
        <p class="text-sm font-medium text-neutral-600 mb-2">Buku Tersedia</p>
