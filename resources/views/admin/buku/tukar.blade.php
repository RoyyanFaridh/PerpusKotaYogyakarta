...
@section('title', 'Buku Tukar')
@section('page-title', 'Buku Tukar')
@section('page-subtitle', 'Buku yang diserahkan masyarakat')
...
<div class="flex flex-col gap-4">
    ...
    <x-admin.page-header
        title="Buku Tukar"
        :subtitle="$stats['total'] . ' buku dari masyarakat'"
        icon="book"
        placeholder="Cari judul, pengarang, ISBN..."
        search-id="searchInput"
        :stats="[
            ['label' => 'Total', 'value' => $stats['total'], 'color' => 'text-neutral-800'],
            ['label' => 'Baik',  'value' => $stats['baik'],  'color' => 'text-success-700'],
            ['label' => 'Cukup', 'value' => $stats['cukup'], 'color' => 'text-warning-700'],
            ['label' => 'Rusak', 'value' => $stats['rusak'], 'color' => 'text-danger-700'],
        ]"
        :filters="[
            [
                'name'        => 'kategori',
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-neutral-100 bg-neutral-50">
                        ...
                        <th class="text-left text-xs font-medium text-neutral-400 px-5 py-3">Kondisi</th>
                ...
                <tbody class="divide-y divide-neutral-50">
                    @forelse ($bukus as $buku)
                        @php
                            $kondisiMap = [
                                'baik'  => 'bg-success-50 text-success-700',
                                'cukup' => 'bg-warning-50 text-warning-700',
                                'rusak' => 'bg-danger-50 text-danger-700',
                            ];
                        @endphp
                        <tr class="hover:bg-neutral-50 transition-colors">
                            ...
                            <td class="px-5 py-3.5 text-xs text-neutral-600 whitespace-nowrap">{{ $buku->pengarang }}</td>
                            <td class="px-5 py-3.5 text-xs font-mono text-neutral-500">{{ $buku->isbn ?? '-' }}</td>
                            <td class="px-5 py-3.5">
                                @if ($buku->kategori)
                                    <span class="text-[0.68rem] font-medium px-2 py-0.5 rounded-full bg-primary-50 text-primary-700">{{ $buku->kategori }}</span>
                                @else
                            ...
                            <td class="px-5 py-3.5">
                                @if ($buku->kondisi)
                                    <span class="text-[0.68rem] font-medium px-2 py-0.5 rounded-full {{ $kondisiMap[$buku->kondisi] ?? '' }}">
                        ...
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-medium text-neutral-500">Belum ada buku tukar</p>
                                    <p class="text-xs text-neutral-400">Buku tukar akan muncul setelah transaksi selesai</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
