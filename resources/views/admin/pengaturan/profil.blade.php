...
<div class="flex flex-col gap-4">
        ...
        <form method="POST" action="{{ route('admin.pengaturan.profil') }}" class="px-6 py-5 flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <input type="text" name="nama" value="{{ old('nama', auth()->user()->nama) }}"
                    class="w-full text-sm px-3.5 py-2.5 rounded-lg border {{ $errors->has('nama') ? 'border-danger-400 bg-danger-50' : 'border-neutral-200' }} text-neutral-800 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 transition">
                @error('nama') <p class="text-[0.68rem] text-danger-500">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <input type="email" name="email" value="{{ old('email', auth()->user()->email) }}" placeholder="contoh@email.com"
                    class="w-full text-sm px-3.5 py-2.5 rounded-lg border {{ $errors->has('email') ? 'border-danger-400 bg-danger-50' : 'border-neutral-200' }} text-neutral-800 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 transition">
                @error('email') <p class="text-[0.68rem] text-danger-500">{{ $message }}</p> @enderror
            </div>

            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-neutral-700">Nomor HP</label>
                <input type="text" name="no_hp" value="{{ old('no_hp', auth()->user()->no_hp) }}" placeholder="08xxxxxxxxxx" maxlength="15"
                    class="w-full text-sm px-3.5 py-2.5 rounded-lg border {{ $errors->has('no_hp') ? 'border-danger-400 bg-danger-50' : 'border-neutral-200' }} text-neutral-800 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 transition">
                @error('no_hp') <p class="text-[0.68rem] text-danger-500">{{ $message }}</p> @enderror
            </div>
    ...
    <div class="relative overflow-hidden rounded-xl bg-white border border-neutral-200">
        ...
        <form method="POST" action="{{ route('admin.pengaturan.password') }}" class="px-6 py-5 flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-neutral-700">Password Saat Ini <span class="text-danger-500">*</span></label>
                <input type="password" name="current_password"
                    class="w-full text-sm px-3.5 py-2.5 rounded-lg border {{ $errors->has('current_password') ? 'border-danger-400 bg-danger-50' : 'border-neutral-200' }} text-neutral-800 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 transition">
                @error('current_password') <p class="text-[0.68rem] text-danger-500">{{ $message }}</p> @enderror
            </div>

            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-neutral-700">Password Baru <span class="text-danger-500">*</span></label>
                <input type="password" name="password" placeholder="Min. 8 karakter"
                    class="w-full text-sm px-3.5 py-2.5 rounded-lg border {{ $errors->has('password') ? 'border-danger-400 bg-danger-50' : 'border-neutral-200' }} text-neutral-800 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400 transition">
                @error('password') <p class="text-[0.68rem] text-danger-500">{{ $message }}</p> @enderror
            </div>

            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-medium text-neutral-700">Konfirmasi Password Baru <span class="text-danger-500">*</span></label>
